...
```
=======================================================
  PorxPy  v0.78.0  (built 2026-08-23)
...
| **Portfolio** | Funds · Cash · History · X-ray · Targets · Holdings · Optimizer | What you actually hold, the look-through analysis of it, your target allocations and the trade solver. |
...
| **C** | bulk | **Import a fund bundle** from Settings → Backup & restore. Many funds at once, with holdings, factsheets, field pins and resource files. Two-phase: you see a conflict table before anything is written. |
...
table usually gives you. The name match is strict on purpose: every word
of the printed name must appear in the candidate's own name, ignoring
"NV", "plc", "Class A" and the like, so a near-miss leaves the row blank
rather than attaching another company's sector to it.
...
| Field | Required | Default value | Yahoo fill |
|---|---|---|---|
| Name | yes | — | — |
| Weight | yes | — | — |
| Ticker | no | — | — |
...
like `PLTRUS`. You do not have to clean the file first. Each row goes
...
2. **ISIN country prefix** — map the first two characters of the ISIN
   (`FR`) to the Yahoo suffix (`.PA`) and try the bare ticker with it.
3. **Identifier search** — pass the CUSIP or ISIN to Yahoo's search
   endpoint.
4. **Name search** — search by security name, matching results against
   the first characters of the raw ticker.

Once resolved, the canonical ticker, ISIN and CUSIP are written back to
the row, so the data is clean going forward.
...
plus four bond columns (Duration, Maturity, Coupon %, Eff. Date) behind a
**▶ Show bond columns** toggle. There is a search box, a per-column
...
its tree, and the column sorts on whichever level is displayed.
...
| **Bond metadata** | Duration (years), Coupon % (per year), Maturity and Effective date, both as `DD/mmm/YYYY`. Usually blank for equity holdings. |
...
A peer group of fewer than **three** funds produces no peer score at all.
A percentile within a group of one is 100 by construction, so every
uniquely positioned fund would score perfectly and be preferentially
swapped in — precisely backwards. Below that size the peer score is `—`
and the optimiser leaves the fund alone on score grounds. It stays fully
...
| **score** | Two figures — universe rank, then peer rank. `—` on the right means the peer group is too small. | yes | weight preset |
